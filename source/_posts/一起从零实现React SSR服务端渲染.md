...
在进行同构渲染的时候，**请务必保证**客户端渲染出来的内容和服务端渲染的内容完全相同。如果客户端和服务端渲染出来的内容不一致，React会尝试对不一致的地方进行修复，而这些修复是[非常耗时的](https://sea.zoom.us/j/7278234400)。如果差异过大甚至会重新渲染整个应用（类似于`ReactDOM.render`）。
